    ...
    try {
      ...
      if (approvedActions.length > 0) {
          ...
          const newBountiesToInsert = approvedActions.map(action => {
            ...
            return {
              name: action.bounty,
              description: originalBounty?.prompt || 'N/A',
              lifespan: 24,
          ...
          const bucketWeights = insertedBounties.map(insertedBounty => {
            const originalAction = approvedActions.find(a => a.bounty === insertedBounty.name);
            if (!originalAction) {
              throw new Error(`Could not find original action for bounty name ${insertedBounty.name}`);
            }
            return {
              bounty_id: insertedBounty.id,
              bucket_id: originalAction.bucket_id,
